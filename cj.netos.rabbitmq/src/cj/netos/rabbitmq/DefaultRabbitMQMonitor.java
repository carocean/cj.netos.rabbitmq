...
public class DefaultRabbitMQMonitor implements IServiceContainerMonitor {
    ...
    protected void scaneConsumerHome(File home, IServiceSite site) throws RabbitMQException {
        ...
        for (File file : files) {
            ...
            consumer.open(file);
            String serviceName = String.format("@.rabbitmq.consumer.%s", name);
            site.addService(serviceName, consumer);
            CJSystem.logging().info(getClass(), String.format("\t成功打开消费者:%s，服务名为:%s。即：可以在程序中作为服务注入使用", name, serviceName));
        }
    }
